class MailObserver {
	observeUnreadInit() {
		if (!this.owa() && !this.consumer_2()) {
			console.log('Missing valid handler, try again in 5 seconds');
			setTimeout(() => this.observeUnreadInit(), 5000);
		}
	}
	...
	owa() {
				...
				if (unread > 0 && !checkonlyzerounread) {
					...
					if (!lastcheck || new Date() - lastcheck > 500) {
						...
						if (!document.hasFocus()) {
							const notification = new Notification('Prospect Mail: New Messages', {
		...
		const observer = new MutationObserver((mutations) => {
			mutations.forEach(() => {
				this.waitForFinalEvent()(checkOwa, 1000, 'mutation detected');
			});
		});
		...
		if (this.owa_timer) {
			clearInterval(this.owa_timer);
		}
		this.owa_timer = setInterval(() => {
			checkOwa(true);
		}, 5000);
...
module.exports = new MailObserver();